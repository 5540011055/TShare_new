...
<div class="box " style="margin-top:50px;border-top: 0px;padding-bottom: 10px;" id="main_component" >
...
<script>
   ...
   //	console.log(type);
      ...
      function viewIncomeDetail(date,driver_id){
      		if(date==''){
   			swal('ไม่มีรายละเอียด','','error');
   		}else{
   			$('#main_load_mod_popup_1').show();
   			$('#text_mod_topic_action_1').text('รายละเอียดรายได้');
   			var url = "load_page_mod_1.php?name=income&file=ic_detail";
   			$.post(url,{ date : date, driver_id:driver_id },function(data){
   				$('#load_mod_popup_1').html(data);
   			});
   		}
      }
